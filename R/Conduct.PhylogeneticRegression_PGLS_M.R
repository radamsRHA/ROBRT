#' Conduct.PhylogeneticRegression_PGLS_M: function to conduct phylogenetic regression using PGLS and M estimator
#'
...
#' @return handle.PGLS_M Object containing results of M regression for PGLS
#' @export
#' @examples
#' 
#' ################
...
#' ######################################
#' Conduct.PhylogeneticRegression_PGLS_M(handle.Phylogeny = handle.Phylogeny, vector.Trait_X = vector.TraitValue_X, vector.Trait_Y = vector.TraitValue_Y)


##########################################
# Conduct.PhylogeneticRegression_PGLS_M #
##########################################
Conduct.PhylogeneticRegression_PGLS_M <- function(handle.Phylogeny, vector.Trait_X, vector.Trait_Y){
  ...
  #########################
  handle.PGLS_M <- rlm(formula = vector.Projected_Trait_Y~matrix.Design_Projected_Trait_X-1, method = "M", scale.est = "Huber", k2 = 1.345)
  
  ##########
  # RETURN #
  ##########
  return(handle.PGLS_M)
